...
<details>
  ...
  **Важно!**
  ...
  В файле `xr_offliner.script` есть локальная переменная: `smartStoryID` её необходимо приравнять к `story_id` любого смрттерейна! В скрипте по умолчанию задан `story_id` смарттерейна из оригинала ЗП!
___________________
